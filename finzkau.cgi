...
$sqlkau ='(SELECT t1.idzak AS idzak,t1.sprnum AS sprnum FROM '.$base.' AS t1 WHERE oc="'.$choice.'" and coc="'.$choicecoc.'") UNION (SELECT t2.idzak AS idzak,t2.sprnum AS sprnum  FROM '.$basesal.' AS t2 WHERE oc="'.$choice.'" and coc="'.$choicecoc.'")';

#$sqlkau ='SELECT DISTINCT idzak,sprnum FROM '.$base.'  WHERE oc="'.$choice.'" and coc="'.$choicecoc.'"';
$sqlkau = $sqlkau . ' ORDER BY idzak';
$sthkau = $dbhr->prepare($sqlkau);
$sthkau->execute();

while (($midzakdis,$sprnum) = $sthkau->fetchrow()){;
my ($idnumbersp,$mshifra,$midnazva)=(1,"0","нет названия");
#my $satel=$sprnum;
if ($midzakdis==0) {
}else{
...
$sqlr ='SELECT id,oc,coc,kc,ckc,booc,data,kau,kaukc,summa,priznak,dok,zakname,idzak FROM '.$base.'  WHERE oc="'.$choice.'" and coc="'.$choicecoc.'" and idzak="'.$midzakdis.'"';
$sqlr = $sqlr . ' ORDER BY idzak,data,kc,ckc,dok';
$sthr = $dbhr->prepare($sqlr);
$sthr->execute();
...
print '<div  style="float:left; width: 12%; text-align: right; background-color: #ffffff;"><font color="#0000ff" size="3">'.sprintf("%19.2f",$psaldebss).'</font></div>';
print '<div  style="float:left; width: 12%; text-align: right; background-color: #ffffff;"><font color="#0000ff" size="3">'.sprintf("%19.2f",$psalkrss).'</font></div>';
#print ' <div  style="clear:both;"></div>';
...
print '<div  style="float:left; width: 12%; text-align: right; background-color: #ffffff;"><font color="#0000ff" size="3">'.sprintf("%19.2f",$isaldebss).'</font></div>';
print '<div  style="float:left; width: 12%; text-align: right; background-color: #ffffff;"><font color="#0000ff" size="3">'.sprintf("%19.2f",$isalkrss).'</font></div>';
print ' <div  style="clear:both; "></div>';
