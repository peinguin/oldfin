...
#$sqlr ='CREATE TABLE fskau331t LIKE f340'; 
...
#$sqlr ='CREATE TABLE f331kor (id int(5) NOT NULL AUTO_INCREMENT PRIMARY KEY)'; 
...
#$sqlr = 'ALTER TABLE f331kor ADD kc varchar(2)';
#$sqlr = 'ALTER TABLE f331kor ADD ckc varchar(2)';
...
#$sqlr = 'ALTER TABLE f331kor ADD kaukc varchar(5)';
#$sqlr = 'ALTER TABLE f331kor ADD sprnumkc int(1)';
#$sqlr = 'ALTER TABLE f330 CHANGE saldo saldo decimal(12,2)';
#$sqlr = 'ALTER TABLE f331kor ADD zaknamekc text(19)';
#$sqlr = 'ALTER TABLE f331kor ADD iduserkc int(5)';
#$sqlr = 'ALTER TABLE f331kor ADD idzakkc INT(10)';
#$sthr = $dbhr->prepare($sqlr);
#$sthr->execute();
...
#print $mid;
#print $mperiod;
print '<form id="formf1op" action="/cgi-bin/fins4red.cgi" method="post">';
...
#if ($mfull eq 2) {
$sqlr ='SELECT SQL_CALC_FOUND_ROWS id,oc,coc,kc,ckc,booc,kau,kaukc,summa,priznak,zakname,zaknamekc FROM '.$base.'';
...
$sqlr = $sqlr . ' ORDER BY oc,coc,kc,ckc,summa' if $idsort eq 'number';
#$sqlr = $sqlr . ' ORDER BY name' if  $idsort eq 'name';

$sqlr = $sqlr . ' ORDER BY oc,coc,summa' if  $idsort eq 'oc';
$sqlr = $sqlr . ' ORDER BY coc,oc,summa' if  $idsort eq 'coc';

$sqlr = $sqlr . ' ORDER BY kc,ckc,summa' if  $idsort eq 'kc';
$sqlr = $sqlr . ' ORDER BY ckc,kc,summa' if  $idsort eq 'ckc';
...
print '<th align="center"><NOBR><a href="/cgi-bin/sortnakl.cgi?psort=oc&redag='.$prym.'&sprnum='.$msp.'&navnakl='.($sdvigpn+1).'&poisk='.$prpoisk.'&id='.$naklid.'" class=winright>_ОC_</a></NOBR></th>';
print '<th align="center"><NOBR><a href="/cgi-bin/sortnakl.cgi?psort=coc&redag='.$prym.'&sprnum='.$msp.'&navnakl='.($sdvigpn+1).'&poisk='.$prpoisk.'&id='.$naklid.'" class=winright>_CОC_</a></NOBR></th>';

print '<th align="center"><NOBR><a href="/cgi-bin/sortnakl.cgi?psort=kc&redag='.$prym.'&sprnum='.$msp.'&navnakl='.($sdvigpn+1).'&poisk='.$prpoisk.'&id='.$naklid.'" class=winright>КC_</a></NOBR></th>';
print '<th align="center"><NOBR><a href="/cgi-bin/sortnakl.cgi?psort=ckoc&redag='.$prym.'&sprnum='.$msp.'&navnakl='.($sdvigpn+1).'&poisk='.$prpoisk.'&id='.$naklid.'" class=winright>_CKC_</a></NOBR></th>';
...
print '<th align="center"><NOBR><a href="/cgi-bin/sortnakl.cgi?psort=obor&redag='.$prym.'&sprnum='.$msp.'&navnakl='.($sdvigpn+1).'&poisk='.$prpoisk.'&id='.$naklid.'" class=winright>Обopoт</a></NOBR></th>';
...
print '<tr bgcolor="#eeeeee"><th align="center"><NOBR><a href="/cgi-bin/sortnakl.cgi?psort=number&redag='.$prym.'&sprnum='.$msp.'&navnakl='.($sdvigpn+1).'&poisk='.$prpoisk.'&id='.$naklid.'" class=winright>d</a></NOBR></th>';
print '<th align="center"><NOBR><a href="/cgi-bin/sortnakl.cgi?psort=number&redag='.$prym.'&sprnum='.$msp.'&navnakl='.($sdvigpn+1).'&poisk='.$prpoisk.'&id='.$naklid.'"  class=winright>________Наименование_________</a></NOBR></th>';
...
#print $prym;
#my $msummaop=0.0;
while (($mrecnum,$moc,$mcoc,$mkc,$mckc,$mbooc,$mkauop,$mkaukc,$msummaop,$mprizop,$mzakname,$mzaknamekc)=$sthr->fetchrow() ){;
$ryadoksp = $ryadoksp+1;
...
#print $idoper;
print  '<a href="/cgi-bin/fins4dubl.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag='.$prym.'&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'&komm2='.$mzakname.'"  class="riaddubl">+</a></td>';
...
print  '<th id=c3'.$keymove.'><a href="/cgi-bin/f1.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag=kc&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" id=p3'.$keymove.'  class="redf1op">'.$mkc.'</a></th>';
...
print  '<th id=c4'.$keymove.'><a href="/cgi-bin/f1.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag=ckc&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" id=p4'.$keymove.'  class="redf1op">'.$mckc.'</a></th>';
...
print '<td id=c5'.$keymove.'>'.$q->popup_menu(-name=>'oborot', -values=>['2','3'], -default=>$mbooc, -labels=>{'2'=>'Дбт','3'=>'Крд'}). '</td>';
...
print  '<th id=c6'.$keymove.'><a style="color:#ff0000; font-size:14pt;" href="/cgi-bin/cleankau.cgi?idkrt='.$num1.'&fbase=34&id='.$mrecnum.'&redag=rkau&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" title='.$zakplat.' id=p6'.$keymove.'  class="cleankau">clean</a></th>';
#print '<td id=c6'.$keymove.'><input type=text size=5 value='.$mkauop.'  id="vvodop" name="parval"></td>';
#print '<input type=hidden  name=id value='.$mrecnum.'>';
#print '<input type=hidden  name=redag value="kau">';
}else{
#$mzakname1=str_replace('"','“',$mzakname);
print  '<th id=c6'.$keymove.'><a href="/cgi-bin/f1.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag=kau&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" title='.$zakplat.' id=p6'.$keymove.'  class="redplat">'.$mkauop.'</a></th>';
}
...
print  '<th id=c7'.$keymove.'><a style="color:#ff0000; font-size:14pt;" href="/cgi-bin/cleankau.cgi?idkrt='.$num1.'&fbase=34&id='.$mrecnum.'&redag=rkaukc&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" title='.$mzaknamekc.' id=p7'.$keymove.'  class="cleankau">clean</a></th>';
#print '<td id=c6'.$keymove.'><input type=text size=5 value='.$mkauop.'  id="vvodop" name="parval"></td>';
#print '<input type=hidden  name=id value='.$mrecnum.'>';
#print '<input type=hidden  name=redag value="kau">';
}else{
#$mzakname1=str_replace('"','“',$mzakname);
print  '<th id=c7'.$keymove.'><a href="/cgi-bin/f1.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag=kaukc&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" title='.$zakplatkc.' id=p7'.$keymove.'  class="redplat">'.$mkaukc.'</a></th>';
}
...
if ($msummaop == 0) {
print '<td id=c8'.$keymove.'><input type=text onblur="this.focus()" id="vvod" size=9 name="parval"></td>';
}else{
print '<td id=c8'.$keymove.'><input type=text onblur="this.focus()" id="vvod" size=9 value='.sprintf("%9.2f",$msummaop).' name="parval"></td>';
}
print '<input type=hidden  name=id value='.$mrecnum.'>';
print '<input type=hidden  name=redag value="summa">';
}else{
if ($msummaop<0) {
print  '<th id=c8'.$keymove.'><a style="color:#ff0000;" href="/cgi-bin/f1.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag=sum&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" id=p8'.$keymove.'  class="redf1op">'.sprintf("%12.2f",$msummaop).'</a></th>';
}else{
print  '<th id=c8'.$keymove.'><a href="/cgi-bin/f1.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag=sum&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" id=p8'.$keymove.'  class="redf1op">'.sprintf("%12.2f",$msummaop).'</a></th>';
}
...
if ($mprizop eq "-") {
print '<td id=c9'.$keymove.'><input type=text size=23 onblur="this.focus()" id="vvod" name="parval"></td>';
}else{
print '<td id=c9'.$keymove.'><input type=text size=23 onblur="this.focus()" id="vvod" value="'.$mprizop.'" name="parval"></td>';
}
...
print  '<th id=c9'.$keymove.' align="left"><a href="/cgi-bin/f1.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag=priznak&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" id=p9'.$keymove.'  class="redf1op">'.$mprizop.'</a></th>';
}
...
#print  '<th id=c10'.$keymove.' align="left"><a href="/cgi-bin/f1.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag=priznak&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" id=p10'.$keymove.'  class="redf1op">'.$mzaknamekc.'</a></th>';
print  '<th id=c10'.$keymove.' align="left"><a href="/cgi-bin/f1.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag=priznak&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" id=p10'.$keymove.'  class="redf1op">'.$mzakname.'</a></th>';
#if ($mfull eq 2) {
...
print  '<td bgcolor="#ddeedd"><a href="/cgi-bin/fins4del.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag='.$prym.'&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" id='.$idmove.'  class="delrriad"> Удалить</td></tr>';
...
while ($ryadok < 19) {;
$ryadok = $ryadok+1;
print  '<tr bgcolor="#eeeeee"><td>..</td>'.'<td> .....</td>'.'<td> .....</td>'.'<td> .....</td>'.'<td> .......</td>'.'<td>.....</td>'.'<td>......</td>'.'<td> ......</td>'.'<td> .........</td>'.'<td> .....</td>'.'<td> ..</td>'.'<td>..</td></tr>';
