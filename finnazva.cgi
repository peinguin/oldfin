...
#$sqlr ='CREATE TABLE fnazrah (id int(5) NOT NULL AUTO_INCREMENT PRIMARY KEY)'; 
#$sthr = $dbhr->do($sqlr);
...
#$sqlr = 'ALTER TABLE fnazrah ADD oc varchar(2)';
#$sqlr = 'ALTER TABLE fnazrah ADD coc varchar(2)';
#$sqlr = 'ALTER TABLE fnazrah ADD rahname text(19)';
#$sqlr = 'ALTER TABLE f330 ADD kau varchar(5)';
#$sqlr = 'ALTER TABLE f330 ADD sprnum int(1)';
#$sqlr = 'ALTER TABLE fnazrah CHANGE rahname rahname text(40)';
...
#print $mid;
#print $mperiod;
print '<form id="formf1op" action="/cgi-bin/fins5red.cgi" method="post">';
...
#if ($mfull eq 2) {
$sqlr ='SELECT SQL_CALC_FOUND_ROWS id,oc,coc,rahname FROM '.$base.'';
...
#$sqlr = $sqlr . ' ORDER BY (data) desc,(id) desc,name, summa' if $idsort eq 'number';
$sqlr = $sqlr . ' ORDER BY oc,coc' if  $idsort eq 'number';

$sqlr = $sqlr . ' ORDER BY oc,coc' if  $idsort eq 'oc';
$sqlr = $sqlr . ' ORDER BY coc,oc' if  $idsort eq 'coc';
$sqlr = $sqlr . ' ORDER BY rahname' if  $idsort eq 'name';
...
print '<th align="center"><NOBR><a href="/cgi-bin/sortnakl.cgi?psort=oc&redag='.$prym.'&sprnum='.$msp.'&navnakl='.($sdvigpn+1).'&poisk='.$prpoisk.'&id='.$naklid.'" id="num" class=winright>__ОC__</a></NOBR></th>';
print '<th align="center"><NOBR><a href="/cgi-bin/sortnakl.cgi?psort=coc&redag='.$prym.'&sprnum='.$msp.'&navnakl='.($sdvigpn+1).'&poisk='.$prpoisk.'&id='.$naklid.'" id="num" class=winright>__CОC__</a></NOBR></th>';

print '<td align="center"><NOBR><a href="/cgi-bin/sortnakl.cgi?psort=name&redag='.$prym.'&sprnum='.$msp.'&navnakl='.($sdvigpn+1).'&poisk='.$prpoisk.'&id='.$naklid.'" class=winright>______________Назва рахунку_____________</a></NOBR></td>';
...
#print $prym;
#my $msummaop=0.0;
while (($mrecnum,$moc,$mcoc,$mzakname)=$sthr->fetchrow() ){;
$ryadoksp = $ryadoksp+1;
...
#print $idoper;
print  '<a href="/cgi-bin/fins5dubl.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag='.$prym.'&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'&komm2='.$mzakname.'"  class="riaddubl">+</a></td>';
...
if (($prym eq "rahname") and $tekriadok==$mrecnum) {
if ($mzakname eq "-") {
print '<td id=c3'.$keymove.'><input type=text size=40 onblur="this.focus()" id="vvod" name="parval"></td>';
}else{
print '<td id=c3'.$keymove.'><input type=text size=40 onblur="this.focus()" id="vvod" value="'.$mzakname.'" name="parval"></td>';
}

#print '<td id=c7'.$keymove.'><input type=text size=23 value='.$mprizop.' name="parval"></td>';
print '<input type=hidden  name=id value='.$mrecnum.'>';
print '<input type=hidden  name=redag value="rahname">';
}else{
print  '<th id=c3'.$keymove.' align="left"><a href="/cgi-bin/f1.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag=rahname&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" id=p3'.$keymove.'  class="redf1op">'.$mzakname.'</a></th>';
}
...
print  '<td bgcolor="#ddeedd"><a href="/cgi-bin/fins5del.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag='.$prym.'&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" id='.$idmove.'  class="delrriad"> Удалить</td></tr>';
...
while ($ryadok < 19) {;
$ryadok = $ryadok+1;
print  '<tr bgcolor="#eeeeee"><td>..</td>'.'<td> .....</td>'.'<td> .......</td>'.'<td> .........</td>'.'<td> ..</td>'.'<td>..</td></tr>';
