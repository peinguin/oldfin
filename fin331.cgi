...
#$sqlr ='CREATE TABLE f331 LIKE f330'; 
...
#$sqlr = 'ALTER TABLE f330 CHANGE summa saldo decimal(12,2)';
#$sqlr = 'ALTER TABLE f330 ADD zakname text(19)';
#$sqlr = 'ALTER TABLE f331kor CHANGE iduser idzak int(5)';
...
#print $mid;
#print $mperiod;
print '<form id="formf1op" action="/cgi-bin/fins3red.cgi" method="post">';
...
#if ($mfull eq 2) {
$sqlr ='SELECT SQL_CALC_FOUND_ROWS id,oc,coc,booc,kau,summa,priznak,zakname FROM '.$base.'';
...
print '<th align="center"><NOBR><a href="/cgi-bin/sortnakl.cgi?psort=oc&redag='.$prym.'&sprnum='.$msp.'&navnakl='.($sdvigpn+1).'&poisk='.$prpoisk.'&id='.$naklid.'" id="num" class=winright>_ОC_</a></NOBR></th>';
...
print '<td align="center"><NOBR><a href="/cgi-bin/sortnakl.cgi?psort=summa&redag='.$prym.'&sprnum='.$msp.'&navnakl='.($sdvigpn+1).'&poisk='.$prpoisk.'&id='.$naklid.'" class=winright>____Сумма___</a></NOBR></td>';
...
#print $idoper;
print  '<a href="/cgi-bin/fins3dubl.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag='.$prym.'&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'&komm2='.$mzakname.'"  class="riaddubl">+</a></td>';
...
print '<td id=c3'.$keymove.'>'.$q->popup_menu(-name=>'oborot', -values=>['2','3'], -default=>$mbooc, -labels=>{'2'=>'Дбт','3'=>'Крд'}). '</td>';
#print '<td id=c3'.$keymove.'>'.$q->popup_menu(-name=>'oborot', -values=>['2','3','4','5'], -default=>$mbooc, -labels=>{'2'=>'Дбт','3'=>'Крд','4'=>'СтД','5'=>'СтК'}). '</td>';
print '<input type=hidden  name=id value='.$mrecnum.'>';
...
print  '<th id=c3'.$keymove.'><a href="/cgi-bin/f1.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag=booc&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" id=p3'.$keymove.'  class="redf1op">'.$tbooc.'</a></th>';
...
print  '<th id=c4'.$keymove.'><a style="color:#ff0000; font-size:14pt;" href="/cgi-bin/cleankau.cgi?idkrt='.$num1.'&fbase=33&id='.$mrecnum.'&redag=rkau&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" title='.$zakplat.' id=p4'.$keymove.'  class="cleankau">clean</a></th>';
...
print  '<th id=c4'.$keymove.'><a href="/cgi-bin/f1.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag=kau&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" title='.$zakplat.' id=p4'.$keymove.'  class="redplat">'.$mkauop.'</a></th>';
}
...
if ($msummaop == 0) {
print '<td id=c5'.$keymove.'><input type=text onblur="this.focus()" id="vvod" size=9 name="parval"></td>';
}else{
print '<td id=c5'.$keymove.'><input type=text onblur="this.focus()" id="vvod" size=9 value='.sprintf("%9.2f",$msummaop).' name="parval"></td>';
}
print '<input type=hidden  name=id value='.$mrecnum.'>';
print '<input type=hidden  name=redag value="summa">';
}else{
if ($msummaop<0) {
print  '<th id=c5'.$keymove.'><a style="color:#ff0000;" href="/cgi-bin/f1.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag=sum&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" id=p5'.$keymove.'  class="redf1op">'.sprintf("%12.2f",$msummaop).'</a></th>';
}else{
print  '<th id=c5'.$keymove.'><a href="/cgi-bin/f1.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag=sum&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" id=p5'.$keymove.'  class="redf1op">'.sprintf("%12.2f",$msummaop).'</a></th>';
}
}
...
if ($mprizop eq "-") {
print '<td id=c6'.$keymove.'><input type=text size=23 onblur="this.focus()" id="vvod" name="parval"></td>';
}else{
print '<td id=c6'.$keymove.'><input type=text size=23 onblur="this.focus()" id="vvod" value="'.$mprizop.'" name="parval"></td>';
}
...
print  '<th id=c6'.$keymove.' align="left"><a href="/cgi-bin/f1.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag=priznak&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" id=p6'.$keymove.'  class="redf1op">'.$mprizop.'</a></th>';
}
...
print  '<th id=c7'.$keymove.' align="left"><a href="/cgi-bin/f1.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag=priznak&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" id=p7'.$keymove.'  class="redf1op">'.$mzakname.'</a></th>';
#if ($mfull eq 2) {
...
print  '<td bgcolor="#ddeedd"><a href="/cgi-bin/fins3del.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag='.$prym.'&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" id='.$idmove.'  class="delrriad"> Удалить</td></tr>';
