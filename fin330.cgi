...
#$sqlr = 'ALTER TABLE f330 CHANGE oc oc int(2)';
...
#$sqlr = 'ALTER TABLE f330new CHANGE iduser idzak int(5)';
...
$sqlr = $sqlr . ' ORDER BY oc,coc,saldo' if $idsort eq 'number';
...
print '<th align="center"><NOBR><a href="/cgi-bin/sortnakl.cgi?psort=obor&redag='.$prym.'&sprnum='.$msp.'&navnakl='.($sdvigpn+1).'&poisk='.$prpoisk.'&id='.$naklid.'" id="num" class=winright>_Оборот</a></NOBR></th>';
...
print '<td align="center"><NOBR><a href="/cgi-bin/sortnakl.cgi?psort=summa&redag='.$prym.'&sprnum='.$msp.'&navnakl='.($sdvigpn+1).'&poisk='.$prpoisk.'&id='.$naklid.'" class=winright>____Сальдо___</a></NOBR></td>';
...
#print $idoper;
print  '<a href="/cgi-bin/fins1dubl.cgi?idkrt='.$num1.'&id='.$mrecnum.'&redag='.$prym.'&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'&komm2='.$mzakname.'"  class="riaddubl">+</a></td>';
...
#print '<td>'.$q->popup_menu(-name=>'.$mrecnum.', -values=>['2','3','4','5'], -default=>$mbooc, -labels=>{'2'=>'Дбт','3'=>'Крд','4'=>'СтД','5'=>'СтК'}). '</td>';
...
$mkauop="clean";
print  '<th id=c4'.$keymove.'><a style="color:#ff0000; font-size:14pt;" href="/cgi-bin/cleankau.cgi?idkrt='.$num1.'&fbase=31&id='.$mrecnum.'&redag=rkau&psort='.$idsort.'&navnakl='.($sdvigpn+1).'&sprnum='.$msp.'" title='.$zakplat.' id=p4'.$keymove.'  class="cleankau">clean</a></th>';
