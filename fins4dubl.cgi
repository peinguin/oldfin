...
$sqlr ='SELECT id,oc,coc,kc,ckc,booc,kau,summa,priznak,zakname FROM '.$base.'  WHERE id="'.$tid.'"';
$sthr = $dbhr->prepare($sqlr);
$sthr->execute();

#print $sqlr;
my ($mrecnum,$moc,$mcoc,$mkc,$mckc,$mbooc,$mkauop,$msummaop,$mprizop,$mzakname)=$sthr->fetchrow();

my $pryzn=$dbhr->quote($mprizop);

my $tname="n";

$sqlr = 'INSERT INTO '.$base.' (oc,coc,kc,ckc,booc,kau,summa,priznak,zakname)';
$sqlr = $sqlr .  ' VALUES ("'.$moc.'","'.$mcoc.'","'.$mkc.'","'.$mckc.'","'.$mbooc.'","'.$mkauop.'","'.$msummaop.'","'.$mprizop.'","'.$mzakname.'")';
